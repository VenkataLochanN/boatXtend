...
public class AdvertisingIdClient {
    ...
    private static zze zza(Context context, BlockingServiceConnection blockingServiceConnection) throws IOException {
        try {
            return zzf.zza(blockingServiceConnection.getServiceWithTimeout(DfuConfig.CONNECTION_PARAMETERS_UPDATE_TIMEOUT, TimeUnit.MILLISECONDS));
        } catch (InterruptedException unused) {
    ...
    private final boolean zza(Info info, boolean z, float f2, long j, String str, Throwable th) {
        ...
        map.put("app_context", z ? "1" : AlexaCustomSkillConstant.EVENT_UN_RECOGNIZE);
        if (info != null) {
            map.put("limit_ad_tracking", info.isLimitAdTrackingEnabled() ? "1" : AlexaCustomSkillConstant.EVENT_UN_RECOGNIZE);
        }
        if (info != null && info.getId() != null) {
            map.put("ad_id_size", Integer.toString(info.getId().length()));
        }
        if (th != null) {
            map.put(AuthorizationResponseParser.ERROR, th.getClass().getName());
        }
