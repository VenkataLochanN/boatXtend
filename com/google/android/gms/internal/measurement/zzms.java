package com.google.android.gms.internal.measurement;

/* JADX INFO: compiled from: com.google.android.gms:play-services-measurement-impl@@17.2.2 */
/* JADX INFO: loaded from: classes.dex */
public final class zzms implements zzdb<zzmv> {
    private static zzms zza = new zzms();
    private final zzdb<zzmv> zzb;

    public static boolean zzb() {
        return ((zzmv) zza.zza()).zza();
    }

    public static boolean zzc() {
        return ((zzmv) zza.zza()).zzb();
    }

    public static boolean zzd() {
        return ((zzmv) zza.zza()).zzc();
    }

    public static boolean zze() {
        return ((zzmv) zza.zza()).zzd();
    }

    public static boolean zzf() {
        return ((zzmv) zza.zza()).zze();
    }

    public static boolean zzg() {
        return ((zzmv) zza.zza()).zzf();
    }

    public static boolean zzh() {
        return ((zzmv) zza.zza()).zzg();
    }

    public static boolean zzi() {
        return ((zzmv) zza.zza()).zzh();
    }

    public static boolean zzj() {
        return ((zzmv) zza.zza()).zzi();
    }

    private zzms(zzdb<zzmv> zzdbVar) {
        this.zzb = zzda.zza((zzdb) zzdbVar);
    }

    public zzms() {
        this(zzda.zza(new zzmu()));
    }

    @Override // com.google.android.gms.internal.measurement.zzdb
    public final /* synthetic */ zzmv zza() {
        return this.zzb.zza();
    }
}