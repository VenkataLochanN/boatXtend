...
public final class VCardResultParser extends ResultParser {
    ...
    private static final Pattern COMMA = Pattern.compile(AppInfo.DELIM);
        ...
        while (i4 < length) {
            ...
            if (iIndexOf < 0) {
                ...
                if (i != 0) {
                    ...
                    if (z2) {
                        strReplaceAll = UNESCAPED_SEMICOLONS.matcher(strReplaceAll).replaceAll(IOUtils.LINE_SEPARATOR_UNIX).trim();
                    }
                } else {
                    if (z2) {
                        strSubstring = UNESCAPED_SEMICOLONS.matcher(strSubstring).replaceAll(IOUtils.LINE_SEPARATOR_UNIX).trim();
                    }
                    strReplaceAll = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(strSubstring).replaceAll("")).replaceAll(IOUtils.LINE_SEPARATOR_UNIX)).replaceAll("$1");
                }
                if ("uri".equals(str3)) {
                    try {
        ...
        for (List<String> list : collection) {
            ...
            if (str != null && !str.isEmpty()) {
                ...
                while (true) {
                    ...
                    if (Intents.WifiConnect.TYPE.equalsIgnoreCase(str2.substring(0, iIndexOf))) {
