...
public final class Detector {
    ...
    private int sampleLine(ResultPoint resultPoint, ResultPoint resultPoint2, int i) {
        ...
        float f2 = fDistance / i;
        float x = resultPoint.getX();
        float y = resultPoint.getY();
        float x2 = ((resultPoint2.getX() - resultPoint.getX()) * f2) / fDistance;
        float y2 = (f2 * (resultPoint2.getY() - resultPoint.getY())) / fDistance;
        int i2 = 0;
    ...
    private int getColor(Point point, Point point2) {
        ...
        for (int i2 = 0; i2 < iCeil; i2++) {
            f2 += x;
            f3 += y;
            if (this.image.get(MathUtils.round(f2), MathUtils.round(f3)) != z) {
                i++;
            }
        }
        float f4 = i / fDistance;
        if (f4 <= 0.1f || f4 >= 0.9f) {
