...
public final class dy {
    ...
    public static String a(String str, String str2) {
        ...
        try {
            byte[] bArrA = r.a(str2.getBytes(Key.STRING_CHARSET_NAME), str);
            int length = bArrA.length;
            StringBuilder sb = new StringBuilder(length * 2);
            for (int i = 0; i < length; i++) {
                sb.append(f5087a[(bArrA[i] >> 4) & 15]);
                sb.append(f5087a[bArrA[i] & DfuConstants.BANK_INFO_NOT_SUPPORTED]);
            }
            return sb.toString();
        } catch (Throwable th) {
    ...
    public static byte[] a(byte[] bArr) throws Exception {
        PublicKey publicKeyGeneratePublic = KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(dl.a(new StringBuilder(new String(f5088b)).reverse().toString().getBytes())));
