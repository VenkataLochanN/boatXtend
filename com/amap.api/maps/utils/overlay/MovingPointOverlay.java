...
public class MovingPointOverlay {
    ...
    private long duration = DfuConfig.CONNECTION_PARAMETERS_UPDATE_TIMEOUT;
    ...
    public IPoint getCurPosition(long j) {
        ...
        while (true) {
            ...
            double dDoubleValue = this.eachDistance.get(i).doubleValue();
            if (d5 > dDoubleValue) {
                d5 -= dDoubleValue;
                i++;
        ...
        return new IPoint((int) (((double) iPoint2.x) + (((double) i2) * d4)), (int) (((double) iPoint2.y) + (((double) i3) * d4)));
    }
    ...
    private float getRotate(IPoint iPoint, IPoint iPoint2) {
        ...
        double d2 = iPoint2.y;
        return (float) ((Math.atan2(((double) iPoint2.x) - ((double) iPoint.x), iPoint.y - d2) / 3.141592653589793d) * 180.0d);
    }
