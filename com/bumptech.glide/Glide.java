        ...
        if (glide == null) {
            synchronized (Glide.class) {
                if (glide == null) {
                    ...
                    List<GlideModule> glideModules = parseGlideModules(applicationContext);
                    Iterator<GlideModule> it = glideModules.iterator();
                    while (it.hasNext()) {
                        it.next().applyOptions(applicationContext, glideBuilder);
                    }
                    glide = glideBuilder.createGlide();
                    Iterator<GlideModule> it2 = glideModules.iterator();
                    while (it2.hasNext()) {
