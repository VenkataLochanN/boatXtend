package com.amazon.identity.auth.device.utils;

import java.io.ByteArrayInputStream;
import java.io.IOException;
import java.security.cert.Certificate;
import java.security.cert.CertificateException;
import java.security.cert.CertificateFactory;

/* JADX INFO: loaded from: classes.dex */
public final class SignatureUtil {
    private static Certificate AMAZON_PUBLIC_CERTIFICATE = null;
    private static final String AMAZON_PUBLIC_CERTIFICATE_STRING = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n";
    private static final String CERTIFICATE_TYPE = "X.509";
    private static final String CHAR_SET = "UTF-8";

    public static Certificate getCertificateFromByteArray(byte[] bArr) throws IOException, CertificateException {
        ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bArr);
        Certificate certificateGenerateCertificate = CertificateFactory.getInstance(CERTIFICATE_TYPE).generateCertificate(byteArrayInputStream);
        byteArrayInputStream.close();
        return certificateGenerateCertificate;
    }

    public static synchronized Certificate getAmazonPublicCertificate() throws IOException, CertificateException {
        if (AMAZON_PUBLIC_CERTIFICATE == null) {
            AMAZON_PUBLIC_CERTIFICATE = getCertificateFromByteArray(AMAZON_PUBLIC_CERTIFICATE_STRING.getBytes("UTF-8"));
        }
        return AMAZON_PUBLIC_CERTIFICATE;
    }
}